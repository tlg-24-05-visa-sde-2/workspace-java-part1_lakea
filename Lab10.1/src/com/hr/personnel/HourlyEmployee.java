...
public class HourlyEmployee extends Employee {
    ...
    public void setRate(double rate) throws IllegalWageException {
        if (rate < FEDERAL_MINIMUM_WAGE) {  // invalid
            throw new IllegalWageException(String.format(
                    "Illegal wage: %s. Federal minimum is %s.", rate, FEDERAL_MINIMUM_WAGE
            ));

//            throw new IllegalArgumentException("Illegal wage: " + rate + ". " +
//                    "Federal minimum is " + FEDERAL_MINIMUM_WAGE + ".");
        }
        this.rate = rate;
    }
